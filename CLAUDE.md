...
**Mandatory workflow:**
0. **First**, call `ToolSearch` with query `"select:falcon_architecture,falcon_file_context,falcon_package_info,falcon_symbol_lookup,falcon_search,falcon_refresh"` to load the falcon tool schemas — they are deferred tools and cannot be called until their schemas are fetched this way.
