...
VIEW_COMMENT = (
    ...
    "numéro JO. Généré par déduction : reponse_id = AN-{date}-{sha1(texte)[:12]} "
    "à l'ingestion (qe/ingestion_an.py). Règle validée contre les "
    "astérisques du PDF officiel du JO. Cross-validation par 2 LLM "
    "sur leg 17 = ~98 % vrais allotissements. Voir docs/rapport_performance_v2.md."
)

REPONSES_COMMENT = (
    "Une ligne par texte de réponse distinct publié au JO. L'id est "
    "synthétique : AN-{YYYYMMDD}-{sha1(texte_reponse)[:12]}. Plusieurs "
    "questions peuvent référencer le même reponse_id (allotissement). "
    "Voir la VUE allotissements_jo pour les groupes ≥ 2 questions."
)
